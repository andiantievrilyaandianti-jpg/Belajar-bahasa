...
function renderLanguageProgress() {
    ...
    container.innerHTML = Object.entries(languageData).map(([key, lang]) => {
        ...
        const completed = userProgress.completedLessons.filter(l => l.startsWith(key)).length;
        const percentage = Math.round((completed / totalLessons) * 100);
        
        return `
            <div class="language-progress-item">
                <div class="language-progress-header">
                    <div class="language-progress-name">
                        <span>${lang.flag}</span>
                        <span>${lang.name}</span>
                    </div>
                    <div class="language-progress-percent">${percentage}%</div>
                </div>
...
function subscribe(plan) {
    userProgress.subscription = plan;
    saveUserProgress();
    updateSubscriptionStatus();
    
    showNotification(`Berhasil berlangganan ${plan.toUpperCase()}! 🎉`, 'success');
    closeSubscription();
    
    // Refresh lessons if in lessons view
    if (currentLanguage) {
        const language = languageData[currentLanguage];
        renderLessons(language.lessons);
    }
...
function updateSubscriptionStatus() {
    ...
    const status = userProgress.subscription;
